...
/* 수신된 메시지를 처리하는 함수 */
void MonitorApp_ProcessMsg(CFE_SB_Buffer_t *BufPtr)
{

    /* 메시지 ID에 따라 처리 */
    // 로그 추가 by juntheworld
    CFE_ES_WriteToSysLog("MonitorApp: Received message: StreamId[0]=0x%02X, StreamId[1]=0x%02X",
                         BufPtr->Msg.CCSDS.Pri.StreamId[0],
                         BufPtr->Msg.CCSDS.Pri.StreamId[1]);
    /* 메시지 내용을 로그로 저장하거나 분석하는 추가 작업을 여기에 추가 가능 */
}
    ...
    */
    while (CFE_ES_RunLoop(&SAMPLE_APP_Data.RunStatus) == true)
    {
        ...
        */
        ...
        if (status == CFE_SUCCESS)
        {
            MonitorApp_ProcessMsg(SBBufPtr);
