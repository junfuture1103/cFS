...
// void additional_critical_operation(void);
// int32 process_task(uint32 mut_id, uint32 sem_id);
// int test_main(void);
...
// SharedResource g_sharedResource = {0, "Initial Message"};
// // 크리티컬 작업 함수들
// void perform_critical_operation() {
//     CFE_ES_SysLogWrite_Unsync("Performing critical operation...\n");
//     g_sharedResource.data += 1;
//     snprintf(g_sharedResource.message, sizeof(g_sharedResource.message), 
//              "Data updated to %d", g_sharedResource.data);
//     CFE_ES_SysLogWrite_Unsync("Updated SharedResource: data = %d, message = '%s'\n", 
//            g_sharedResource.data, g_sharedResource.message);
// }

// void additional_critical_operation() {
//     CFE_ES_SysLogWrite_Unsync("Performing additional critical operation...\n");
//     if (g_sharedResource.data % 2 == 0) {
//         snprintf(g_sharedResource.message, sizeof(g_sharedResource.message), 
//                  "Data %d is even", g_sharedResource.data);
//     } else {
//         snprintf(g_sharedResource.message, sizeof(g_sharedResource.message), 
//                  "Data %d is odd", g_sharedResource.data);
//     }
//     CFE_ES_SysLogWrite_Unsync("Updated SharedResource: data = %d, message = '%s'\n", 
//            g_sharedResource.data, g_sharedResource.message);
// }
...
// int32 process_task(uint32 mut_id, uint32 sem_id) {
//     // 뮤텍스 획득
//     int32 status = OS_MutSemTake(mut_id);
...
//     // 뮤텍스를 보유한 상태에서 세마포어 대기 (문제가 있는 부분)
//     status = OS_BinSemTake(sem_id);
//     if (status != OS_SUCCESS) {
//         OS_MutSemGive(mut_id); // 뮤텍스 해제
//         return status;
//     }

//     additional_critical_operation();
//     // 보호된 영역 종료
...
//     // 뮤텍스 생성
//     status = OS_MutSemCreate(&mutex_id, "SharedResourceMutex", 0);
//     if (status != OS_SUCCESS) {
//         CFE_ES_SysLogWrite_Unsync("Failed to create mutex.\n");
//         return -1;
//     }

//     // 이진 세마포어 생성 (가정)
//     status = OS_BinSemCreate(&sem_id, "SyncSemaphore", 0, 0);
//     if (status != OS_SUCCESS) {
//         CFE_ES_SysLogWrite_Unsync("Failed to create binary semaphore.\n");
//         return -1;
//     }

//     // 프로세스 태스크 실행
//     status = process_task(mutex_id, sem_id);
//     if (status != OS_SUCCESS) {
//         CFE_ES_SysLogWrite_Unsync("process_task failed.\n");
//     }

//     // 뮤텍스 및 세마포어 삭제
//     OS_MutSemDelete(mutex_id);
//     OS_BinSemDelete(sem_id);
...
CFE_Status_t CFE_ES_ReloadApp(CFE_ES_AppId_t AppID, const char *AppFileName)
{
    ...
    if (AppRecPtr != NULL)
    {
        CFE_ES_SysLogWrite_Unsync("CFE_ES_ReloadApp: waiting for mutax return..\n");
        CFE_ES_LockSharedData(__func__, __LINE__);
