...
void perform_critical_operation() {
    CFE_ES_WriteToSysLog("Performing critical operation...\n");
    ...
    CFE_ES_WriteToSysLog("Updated SharedResource: data = %d, message = '%s'\n", 
           g_sharedResource.data, g_sharedResource.message);
}

void additional_critical_operation() {
    CFE_ES_WriteToSysLog("Performing additional critical operation...\n");
...
int test_main(void) {
    ...
    if (status != OS_SUCCESS) {
        CFE_ES_WriteToSysLog("Failed to create mutex.\n");
        return -1;
    }
    ...
    if (status != OS_SUCCESS) {
        CFE_ES_WriteToSysLog("Failed to create binary semaphore.\n");
        return -1;
    }
    ...
    if (status != OS_SUCCESS) {
        CFE_ES_WriteToSysLog("process_task failed.\n");
    }
