...
void CFE_SB_TransmitTxn_FindDestinations(CFE_SB_MessageTxn_State_t *TxnPtr, CFE_SB_BufferD_t *BufDscPtr)
{
    ...
    // by juntheworld
    char AppNameBuf[OS_MAX_API_NAME];
    char DestAppNameBuf[OS_MAX_API_NAME];

    CFE_ES_GetAppName(AppNameBuf, AppId, sizeof(AppNameBuf));
    CFE_ES_WriteToSysLog("[2] Current AppId = 0x%04X(%s) Dest = 0x%04x\n", AppId, AppNameBuf, CFE_SB_MsgIdToValue(TxnPtr->RoutingMsgId));
        ...
        while (DestPtr != NULL && TxnPtr->NumPipes < TxnPtr->MaxPipes)
        {
            ...
            if (CFE_SB_PipeDescIsMatch(PipeDscPtr, DestPtr->PipeId))
            {
                    ...
                    !CFE_RESOURCEID_TEST_EQUAL(PipeDscPtr->AppId, AppId))
                {
                    ...
                    CFE_ES_WriteToSysLog("[3-1] SB: Sending message to FromAppId=0x%04X, PipeId=0x%04X, SysQueueId=0x%04X, AppId=0x%04X\n", 
                        AppId, DestPtr->PipeId, PipeDscPtr->SysQueueId, PipeDscPtr->AppId);

                    CFE_ES_GetAppName(DestAppNameBuf, PipeDscPtr->AppId, sizeof(DestAppNameBuf));
                    CFE_ES_WriteToSysLog("[3-2] Current AppId = 0x%04X(%s) Dest = 0x%04x(%s)\n", AppId, AppNameBuf, PipeDscPtr->AppId, DestAppNameBuf);
...
void CFE_SB_TransmitTxn_Execute(CFE_SB_MessageTxn_State_t *TxnPtr, CFE_SB_Buffer_t *BufPtr)
{
    ...
    // 로그 추가 by juntheworld
    // Actually CCSDS Header info
    CFE_ES_WriteToSysLog("======== new message send!!! ====\n");
    CFE_ES_WriteToSysLog("[1] CFE_SB_TransmitTxn_Execute: StreamId=0x%02X%02X, Sequence=0x%02X%02X, Length=0x%02X%02X\n",
                         BufPtr->Msg.CCSDS.Pri.StreamId[0],
                         BufPtr->Msg.CCSDS.Pri.StreamId[1],
                         BufPtr->Msg.CCSDS.Pri.Sequence[0],
                         BufPtr->Msg.CCSDS.Pri.Sequence[1],
                         BufPtr->Msg.CCSDS.Pri.Length[0],
                         BufPtr->Msg.CCSDS.Pri.Length[1]
                         );

    // 로그 추가 by juntheworld
    // for get the funcCode -> Command(MSG) ID
    CFE_MSG_GetMsgId(&BufPtr->Msg, &MsgId);
    CFE_MSG_GetSize(&BufPtr->Msg, &MsgSize);
    CFE_MSG_GetFcnCode(&BufPtr->Msg, &MsgFc);

    CFE_ES_WriteToSysLog("[1-2] Msg ID = 0x%X,  FunCode = %u, Len = %u",
    ...
    uint8_t *bytePtr = (uint8_t *)&BufPtr->Msg;
